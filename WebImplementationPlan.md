...
- [x] **10.1 Export** (`js/export.js` + Export Roster section in the Help panel, role matrix):
  ...
  - **Shareable table polish (user feedback):** both PNG and PDF now use proper table formatting - Group | Members tables per day (Reserves row highlighted), a full Name | Class | Role guild table, bordered gridlines, alternating row shading, and an outer frame on the image; print CSS adds repeating table headers + row break protection. The Help-panel CSV-player-list and JSON buttons were removed (redundant): guild CSV export/import lives near the Guild panel and JSON backup lives in Admin Tools
